...
%PARAMCONFIGBUTTONPUSHED Check param links before export; prompt for missing ones.
%   If all component instances have valid param file links, exports
%   immediately. Otherwise shows a modal dialog listing missing links
%   with a "Link..." button for each.
    ...
    % If nothing is missing, export directly
    if isempty(missingLabels)
        doExport(app);
        return;
    end

    % Otherwise show a link dialog
    showLinkDialog(app, missingLabels, missingKeys);
end
...
%DOEXPORT Run the param script export.
    try
...
function showLinkDialog(app, missingLabels, missingKeys)
%SHOWLINKDIALOG Display a modal panel inside the app for linking missing param files.
...
%ONDONE Validate all links, export if complete, close dialog.
    compKeys = fieldnames(app.ComponentDropdowns);
    stillMissing = {};
...
%ONCANCEL Close dialog without exporting.
    closeDialog(dlg, storedStates);
    uialert(app.UIFigure, ...
        sprintf('Still missing param file for some components.\nAborting param script creation.'), ...
        'Missing Param', 'Icon', 'warning');
    uiresume(app.UIFigure);
end
