...
vehicleThermal.coolant_T_init=25;  % Coolant initial temperature
vehicleThermal.CabinSpTp=20;       % Cabin setpoint temperature
vehicleThermal.cabin_T_init=25;    % Cabin initial temperature
...
battery.AH=34;    % Cell capacity for 40KWh battery
...
% Battery setting
battery.T_vec=[278 293 313];                 % Temperature vector T [K]
battery.AH=37.5;    % Cell capacity for 45KWh battery
battery.AH_vec=[0.9*battery.AH battery.AH 0.9*battery.AH];    % Cell capacity vector AH(T) [Ahr]
battery.SOC_vec=[0, .1, .25, .5, .75, .9, 1]; % Cell state of charge vector SOC [-]
battery.initialPackSOC=0.75;	% Pack intial SOC (-)
% Run parameter file for battery
